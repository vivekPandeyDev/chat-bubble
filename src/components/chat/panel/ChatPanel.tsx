...
const ChatPanel = ({ selectedRoomId }: ChatPanelProps) => {
  console.error('selectedRoomId in ChatPanel:', selectedRoomId);
  ...
  .sort((a, b) => a.page - b.page) // ensure pages are in order
  .flatMap(page => page.content)   // get all messages in one array
  .sort((a, b) => new Date(b.sentAt).getTime() - new Date(a.sentAt).getTime()); // optional: sort by sentAt
